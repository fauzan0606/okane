...
type RawTransaction = Prisma.TransactionGetPayload<{ include: { wallet: { include: { currency: true } }; category: true; payee: true; installmentPlan: true } }>;
...
export async function getTransactions() {
  const transactions = await prisma.transaction.findMany({ where: { kind: "STANDARD" }, include: { wallet: { include: { currency: true } }, category: true, payee: true, installmentPlan: true }, orderBy: { transactionDate: "desc" } });
  return transactions.map(serializeTransaction);
}

export async function getTransactionById(id: string) {
  const transaction = await prisma.transaction.findFirst({ where: { id, kind: "STANDARD" }, include: { wallet: { include: { currency: true } }, category: true, payee: true, installmentPlan: true } });
