...
export default function TransactionForm({ mode, transaction, wallets, categories, subcategories, payees, trigger, reviewMode = false, sameMerchantCount = 0 }: TransactionFormProps) {
  ...
  const [reimbursementEnabled, setReimbursementEnabled] = useState(transaction?.kind === "REIMBURSEMENT" && transaction?.type === "EXPENSE");
  ...
  function resetCreateForm() { setFormKey((k) => k + 1); setWalletId(""); setType(TRANSACTION_TYPES[0]); setCategoryId(""); setSubcategoryId(""); setAmount(""); setInstallmentEnabled(false); setInstallmentTenor("6"); setInstallmentStartDate(dateInputValue()); setInstallmentFee("0"); setReimbursementEnabled(false); }
        ...
        <div className="p-5 sm:p-7 md:p-8"><DialogHeader className="mb-5 flex-row items-start gap-4 pr-10 sm:mb-6"><div><DialogTitle className="text-xl font-semibold text-white sm:text-2xl">{mode === "create" ? "Add Transaction" : "Edit Transaction"}</DialogTitle><DialogDescription className="mt-1.5 text-xs leading-5 text-slate-500 sm:text-sm sm:leading-6">Record an income or expense.</DialogDescription></div></DialogHeader>
          <form key={formKey} action={formAction} className="overflow-hidden rounded-[22px] border border-[#30465D] bg-[#0A1119]">
            ...
            <div className="space-y-4 p-4 sm:space-y-5 sm:p-6">
              ...
              <div className="space-y-1.5"><Label htmlFor="walletId">Wallet</Label>              {type === "EXPENSE" && <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.04] p-4"><label className="flex cursor-pointer items-start gap-3"><input type="checkbox" name="reimbursementEnabled" value="true" checked={reimbursementEnabled} onChange={(event) => setReimbursementEnabled(event.target.checked)} className="mt-1 h-4 w-4 rounded border-white/20 bg-transparent accent-emerald-500" /><span><span className="block text-sm font-semibold text-white">Akan direimburse</span><span className="mt-0.5 block text-xs text-slate-500">Tandai jika pengeluaran ini dibayar dulu menggunakan uang pribadi dan akan diganti oleh kantor/orang lain.</span></span></label></div>}
<Select name="walletId" value={walletId} onValueChange={(value) => { const nextValue = value ?? ""; setWalletId(nextValue); if (wallets.find((wallet) => wallet.id === nextValue)?.walletType !== "CREDIT_CARD") setInstallmentEnabled(false); }}><SelectTrigger id="walletId" className="w-full"><SelectValue placeholder="Select wallet">{selectedWallet?.name ?? "Select wallet"}</SelectValue></SelectTrigger><SelectContent>{wallets.map((wallet) => <SelectItem key={wallet.id} value={wallet.id}>{wallet.name}</SelectItem>)}</SelectContent></Select>{state.fieldErrors?.walletId && <p className="text-xs text-destructive">{state.fieldErrors.walletId[0]}</p>}</div>
            ...
            </div><DialogFooter className={`grid gap-2 border-t border-[#30465D] bg-[#0E1925] p-4 sm:p-5 ${canApplyToSameMerchant ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}><Button type="button" className="w-full" variant="outline" onClick={() => { resetCreateForm(); setOpen(false); }} disabled={isPending || isBulkPending}>Cancel</Button>{canApplyToSameMerchant && <Button type="button" className="w-full" variant="outline" onClick={applyToSameMerchant} disabled={isPending || isBulkPending}>Apply to all {sameMerchantCount} same-merchant transactions</Button>}<Button type="submit" className="w-full" disabled={isPending || isBulkPending}>{isPending ? "Saving..." : mode === "create" ? "Create Transaction" : "Save only this transaction"}</Button></DialogFooter>
          </form>
        </div>
      </DialogContent>
    </Dialog>
