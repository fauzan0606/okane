      ...
      <DialogContent showCloseButton className="!w-[calc(100vw-1.5rem)] !max-w-[760px] max-h-[92vh] overflow-y-auto rounded-[26px] border border-[#30465D] bg-[#0E1925] p-0 text-white shadow-[0_24px_70px_rgba(0,0,0,0.45)] sm:!w-[calc(100vw-3rem)]">
        <div className="p-5 sm:p-7 md:p-8">
          <DialogHeader className="mb-5 flex-row items-start gap-4 pr-10 sm:mb-6">
            <div>
              <DialogTitle className="text-xl font-semibold text-white sm:text-2xl">{mode === "create" ? "Add Transaction" : "Edit Transaction"}</DialogTitle>
              <DialogDescription className="mt-1.5 text-xs leading-5 text-slate-500 sm:text-sm sm:leading-6">Record an income or expense.</DialogDescription>
            </div>
          </DialogHeader>
          <form key={formKey} action={formAction} className="overflow-hidden rounded-[22px] border border-[#30465D] bg-[#0A1119]">
            ...
            <div className="space-y-4 p-4 sm:space-y-5 sm:p-6">
              ...
              {canInstallment && <div className="rounded-2xl border border-[#30465D] bg-[#172A3D] p-4">
                <label className="flex cursor-pointer items-start gap-3"><input type="checkbox" name="installmentEnabled" value="true" checked={installmentEnabled} onChange={(event) => setInstallmentEnabled(event.target.checked)} className="mt-1 h-4 w-4 rounded border-white/20 bg-transparent accent-emerald-500" /><span><span className="block text-sm font-semibold text-white">Installment</span><span className="mt-0.5 block text-xs text-slate-500">Create a recurring credit-card installment plan. Off by default.</span></span></label>
                {installmentEnabled && <div className="mt-4 grid gap-4 md:grid-cols-2">
                  ...
                  <div className="rounded-xl border border-[#30465D] bg-black/10 px-3 py-2"><p className="text-[10px] uppercase tracking-[0.08em] text-slate-500">Estimated monthly</p><p className="mt-1 text-sm font-semibold text-emerald-300">Rp{monthlyAmount.toLocaleString("id-ID", { maximumFractionDigits: 2 })}</p><p className="mt-0.5 text-[10px] text-slate-600">Total includes optional fee.</p></div>
                </div>}
