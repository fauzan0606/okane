...
function hasCategoryIssue(row: EditableTransaction, subcategories: Subcategory[]) {
  return !row.categoryId || Boolean(row.subcategoryId && !subcategories.some((s) => s.id === row.subcategoryId && s.categoryId === row.categoryId));
}
...
export default function TransactionImport({ wallets, categories, subcategories }: Props) {
  ...
  const duplicateCounts = useMemo(() => {
    ...
    for (const r of rows) {
      const k = normalizeDescription(r.merchant);
  ...
  const visibleRows = useMemo(() => {
    switch (reviewFilter) {
      case "REQUIRED":
        return rows.filter((row) => hasRequiredIssue(row));
      case "CATEGORY":
        return rows.filter((row) => hasCategoryIssue(row, subcategories));
      default:
        return rows;
    }
  }, [reviewFilter, rows, subcategories]);
          ...
          {rows.length > 0 && <>
            <div className="mt-5 rounded-2xl border border-amber-400/15 bg-amber-400/[0.04] p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div><p className="text-sm font-semibold text-amber-100">Review before saving</p><p className="mt-1 text-xs text-amber-200/70">Detected {rows.length} transactions. Nothing is saved yet.</p></div>
                <div className="flex flex-wrap gap-2 text-[10px] font-semibold">
                  <button type="button" onClick={() => changeReviewFilter("ALL")} className={`rounded-full border px-2.5 py-1 ${reviewFilter === "ALL" ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-200" : "border-white/10 bg-[#0A1119] text-slate-300 hover:bg-white/[0.05]"}`}>All {rows.length}</button>
                  <button type="button" onClick={() => changeReviewFilter("REQUIRED")} disabled={requiredIssues === 0} className={`rounded-full border px-2.5 py-1 ${reviewFilter === "REQUIRED" ? "border-red-400/30 bg-red-400/15 text-red-200" : requiredIssues > 0 ? "border-red-400/15 bg-red-400/10 text-red-300 hover:bg-red-400/15" : "cursor-not-allowed border-white/10 bg-[#0A1119] text-slate-600"}`}>Required review {requiredIssues}</button>
                  <button type="button" onClick={() => changeReviewFilter("CATEGORY")} disabled={categoryIssues === 0} className={`rounded-full border px-2.5 py-1 ${reviewFilter === "CATEGORY" ? "border-amber-400/30 bg-amber-400/15 text-amber-100" : categoryIssues > 0 ? "border-amber-400/15 bg-amber-400/10 text-amber-200 hover:bg-amber-400/15" : "cursor-not-allowed border-white/10 bg-[#0A1119] text-slate-600"}`}>Category review {categoryIssues}</button>
                </div>
              </div>
            </div>

            {reviewFilter !== "ALL" && <div className={`mt-3 rounded-xl border px-3 py-2 text-xs ${reviewFilter === "REQUIRED" ? "border-red-400/20 bg-red-400/[0.04] text-red-200" : "border-amber-400/20 bg-amber-400/[0.04] text-amber-200"}`}>
              Showing <strong>{visibleRows.length}</strong> of <strong>{rows.length}</strong> transactions requiring <strong>{filterLabel.toLowerCase()}</strong>. Use <button type="button" onClick={() => changeReviewFilter("ALL")} className="font-semibold underline underline-offset-2">Show all</button> to return to the full list.
            </div>}
            ...
            <div key={reviewFilter} className="mt-3 space-y-3">
              {visibleRows.map((row) => {
