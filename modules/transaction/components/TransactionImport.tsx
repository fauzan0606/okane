...
export default function TransactionImport({ wallets, categories, subcategories }: Props) {
  ...
  async function scan(file: File) {
    ...
    try {
      ...
      const data = await response.json() as { error?: string; transactions?: ImportedTransaction[] };
  ...
  async function importRows() {
    ...
    try {
      ...
      setRows([]); setFileName(""); setOpen(false); window.location.reload();
    } catch (importError) {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen && !loading && !importing) { setRows([]); setFileName(""); setError(""); } setOpen(isOpen); }}>
      ...
      <DialogContent showCloseButton className="!w-[calc(100vw-1.5rem)] !max-w-[1180px] max-h-[92vh] overflow-y-auto rounded-[26px] border border-[#30465D] bg-[#0E1925] p-0 text-white">
        <div className="p-5 sm:p-7">
          <DialogHeader>
            ...
            <DialogDescription className="mt-1.5 text-xs leading-5 text-slate-500 sm:text-sm">Upload an image, PDF, CSV or TXT file. OKANE extracts the transactions first, then shows a review box before anything is written to the database.</DialogDescription>
          </DialogHeader>
          ...
          <label className="mt-5 flex cursor-pointer items-center justify-center gap-2 rounded-2xl border border-dashed border-[#405A74] bg-[#0A1119] px-5 py-6 text-sm font-semibold text-slate-300 hover:border-emerald-400/40">
            <input type="file" accept="image/*,.pdf,.csv,.txt,text/csv,application/pdf" className="hidden" disabled={loading || importing} onChange={(event) => { const file = event.target.files?.[0]; if (file) void scan(file); }} />
          ...
          <DialogFooter className="mt-5 border-t border-white/5 pt-5">
            <Button type="button" variant="outline" onClick={() => { setRows([]); setFileName(""); setError(""); setOpen(false); }} disabled={loading || importing}>Cancel</Button>
