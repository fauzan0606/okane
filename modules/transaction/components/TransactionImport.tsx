...
function hasRequiredIssue(row: EditableTransaction) { return !row.date || !row.merchant || !row.walletId || !(row.amount > 0); }
function hasCategoryIssue(row: EditableTransaction, subcategories: Subcategory[]) { return !row.categoryId || Boolean(row.subcategoryId && !subcategories.some((s) => s.id === row.subcategoryId && s.categoryId === row.categoryId)); }
...
export default function TransactionImport({ wallets, categories, subcategories }: Props) {
  ...
  const categoryIssues = useMemo(() => rows.filter((r) => hasCategoryIssue(r, subcategories)).length, [rows, subcategories]);
  const incompleteRows = useMemo(() => rows.filter((r) => !r.date || !r.merchant || !(r.amount > 0)), [rows]);
  ...
  const visibleRows = useMemo(() => reviewFilter === "REQUIRED" ? rows.filter(hasRequiredIssue) : reviewFilter === "CATEGORY" ? rows.filter((r) => hasCategoryIssue(r, subcategories)) : rows, [reviewFilter, rows, subcategories]);

  function updateRow(sourceIndex: number, patch: Partial<EditableTransaction>) { setRows((current) => current.map((r) => r.sourceIndex === sourceIndex ? { ...r, ...patch } : r)); }
  function updateCategory(sourceIndex: number, categoryId: string) { updateRow(sourceIndex, { categoryId, subcategoryId: "" }); }
  function applyCategoryToSameDescription(sourceIndex: number) {
    ...
    const key = normalizeDescription(target.merchant); if (!key) return;
  ...
  async function scan(file: File) {
    ...
    try {
      ...
      const imported = (data.transactions ?? []).map((r) => ({ ...r, walletId: r.suggestedWalletId ?? "", categoryId: r.suggestedCategoryId ?? "", subcategoryId: r.suggestedSubcategoryId ?? "", selected: true }));
      ...
      else if (imported.some((r) => !r.date || !r.merchant || !(r.amount > 0))) setShowIncompletePrompt(true);
    } catch (e) { setError(e instanceof Error ? e.message : "Could not read the transaction file."); }
    finally { setLoading(false); }
  }

  function deleteIncompleteRows() {
    setRows((current) => current.filter((r) => r.date && r.merchant && r.amount > 0));
    setShowIncompletePrompt(false); if (reviewFilter === "REQUIRED") setReviewFilter("ALL");
    ...
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen && !loading && !importing) resetImportState(); setOpen(isOpen); }}>
      ...
      <DialogContent showCloseButton className="!w-[calc(100vw-1.5rem)] !max-w-[1180px] max-h-[92vh] overflow-y-auto rounded-[26px] border border-[#30465D] bg-[#0E1925] p-0 text-white">
        <div className="p-5 sm:p-7">
          ...
          {rows.length > 0 && <>
            <div className="mt-5 rounded-2xl border border-amber-400/15 bg-amber-400/[0.04] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-amber-100">Review before saving</p><p className="mt-1 text-xs text-amber-200/70">Detected {rows.length} transaction{rows.length === 1 ? "" : "s"}. Nothing is saved yet.</p></div><div className="flex flex-wrap gap-2 text-[10px] font-semibold"><button type="button" onClick={() => setReviewFilter("ALL")} className={`rounded-full border px-2.5 py-1 ${reviewFilter === "ALL" ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-200" : "border-white/10 bg-[#0A1119] text-slate-300 hover:bg-white/[0.05]"}`}>All {rows.length}</button>{requiredIssues > 0 && <button type="button" onClick={() => setReviewFilter("REQUIRED")} className={`rounded-full border px-2.5 py-1 ${reviewFilter === "REQUIRED" ? "border-red-400/30 bg-red-400/15 text-red-200" : "border-red-400/15 bg-red-400/10 text-red-300 hover:bg-red-400/15"}`}>Required review {requiredIssues}</button>}{categoryIssues > 0 && <button type="button" onClick={() => setReviewFilter("CATEGORY")} className={`rounded-full border px-2.5 py-1 ${reviewFilter === "CATEGORY" ? "border-amber-400/30 bg-amber-400/15 text-amber-100" : "border-amber-400/15 bg-amber-400/10 text-amber-200 hover:bg-amber-400/15"}`}>Category review {categoryIssues}</button>}</div></div></div>
            ...
            <div className="mt-4 flex items-center justify-between gap-3 text-[10px] text-slate-500"><span>{reviewFilter === "ALL" ? `Showing all ${rows.length} transactions` : `Showing ${visibleRows.length} ${reviewFilter === "CATEGORY" ? "category review" : "required review"} transaction${visibleRows.length === 1 ? "" : "s"}`}</span>{reviewFilter !== "ALL" && <button type="button" onClick={() => setReviewFilter("ALL")} className="rounded-lg border border-white/10 px-2.5 py-1.5 font-semibold text-slate-300 hover:bg-white/[0.05]">Show all transactions</button>}</div>

            <div className="mt-3 space-y-3">{visibleRows.map((row) => {
              const visibleCategories = categories.filter((c) => c.type === row.type);
              const visibleSubcategories = subcategories.filter((s) => s.categoryId === row.categoryId);
              ...
              return <div key={row.sourceIndex} className={`rounded-2xl border bg-[#101B28] p-4 ${requiredMissing ? "border-red-400/20" : categoryChanged || subcategoryChanged ? "border-amber-400/15" : "border-white/5"}`}>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5"><label className="block"><span className="mb-1.5 block text-[9px] uppercase tracking-[0.1em] text-slate-500">Date</span><input type="date" value={row.date} onChange={(e) => updateRow(row.sourceIndex, { date: e.target.value })} className="w-full rounded-xl border border-white/10 bg-[#07101A] px-3 py-2.5 text-xs text-white outline-none" /></label><label className="block"><span className="mb-1.5 block text-[9px] uppercase tracking-[0.1em] text-slate-500">Merchant</span><input value={row.merchant} onChange={(e) => updateRow(row.sourceIndex, { merchant: e.target.value })} className="w-full rounded-xl border border-white/10 bg-[#07101A] px-3 py-2.5 text-xs text-white outline-none" /></label><label className="block"><span className="mb-1.5 block text-[9px] uppercase tracking-[0.1em] text-slate-500">Amount</span><input type="number" min="0" step="0.01" value={row.amount || ""} onChange={(e) => updateRow(row.sourceIndex, { amount: Number(e.target.value) })} className="w-full rounded-xl border border-white/10 bg-[#07101A] px-3 py-2.5 text-xs text-white outline-none" /></label><label className={`block ${!row.walletId ? "rounded-xl border border-red-400/20 p-2" : ""}`}><span className="mb-1.5 block text-[9px] uppercase tracking-[0.1em] text-slate-500">Wallet</span><select value={row.walletId} onChange={(e) => updateRow(row.sourceIndex, { walletId: e.target.value })} className="w-full rounded-xl border border-white/10 bg-[#07101A] px-3 py-2.5 text-xs text-white outline-none"><option value="">Select wallet</option>{wallets.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select></label><label className={`block ${!row.categoryId || categoryChanged ? "rounded-xl border border-amber-400/20 p-2" : ""}`}><span className="mb-1.5 block text-[9px] uppercase tracking-[0.1em] text-slate-500">Category</span><select value={row.categoryId} onChange={(e) => updateCategory(row.sourceIndex, e.target.value)} className="w-full rounded-xl border border-white/10 bg-[#07101A] px-3 py-2.5 text-xs text-white outline-none"><option value="">Select category</option>{visibleCategories.map((c) => <option key={c.id} value={c.id}>{c.icon ? `${c.icon} ` : ""}{c.name}</option>)}</select></label></div>
                <div className="mt-3 grid gap-3 md:grid-cols-2"><label className={`block ${subcategoryChanged ? "rounded-xl border border-amber-400/20 p-2" : ""}`}><span className="mb-1.5 block text-[9px] uppercase tracking-[0.1em] text-slate-500">Subcategory</span><select value={row.subcategoryId} onChange={(e) => updateRow(row.sourceIndex, { subcategoryId: e.target.value })} disabled={!row.categoryId} className="w-full rounded-xl border border-white/10 bg-[#07101A] px-3 py-2.5 text-xs text-white outline-none"><option value="">{row.categoryId ? "Select subcategory" : "Select category first"}</option>{visibleSubcategories.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label className="block"><span className="mb-1.5 block text-[9px] uppercase tracking-[0.1em] text-slate-500">Note</span><input value={row.note} onChange={(e) => updateRow(row.sourceIndex, { note: e.target.value })} className="w-full rounded-xl border border-white/10 bg-[#07101A] px-3 py-2.5 text-xs text-white outline-none" /></label></div>
