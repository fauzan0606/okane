...
export default function CreditCardStatementCard({ statement }: { statement: CreditCardStatement }) {
  ...
  return (
    <div className="rounded-[20px] border border-white/10 bg-[#0E151E] p-5 shadow-[0_12px_35px_rgba(0,0,0,0.16)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-white">
            {new Date(statement.statementDate).toLocaleDateString("id-ID", { day: "2-digit", month: "long", year: "numeric" })}
          </p>
      ...
      {statement.paidAt && (
        <p className="mt-2 text-xs text-slate-500">Paid on {new Date(statement.paidAt).toLocaleDateString("id-ID")}</p>
      )}
