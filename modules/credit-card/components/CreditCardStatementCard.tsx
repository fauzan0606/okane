...
export default function CreditCardStatementCard({ statement }: { statement: CreditCardStatementView }) {
  ...
  const statusClass = statement.status === "PAID" ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-300" : statement.status === "OVERDUE" ? "border-red-400/25 bg-red-400/10 text-red-300" : statement.status === "PARTIALLY_PAID" ? "border-amber-400/20 bg-amber-400/10 text-amber-300" : "border-white/10 bg-white/5 text-slate-300";
  ...
  return (
    <div className="rounded-[20px] border border-white/10 bg-[#0E151E] p-5 shadow-[0_12px_35px_rgba(0,0,0,0.16)]">
      ...
      {statement.paidAt && <p className="mt-2 text-xs text-slate-500">Paid on {new Date(statement.paidAt).toLocaleDateString("id-ID")}</p>}
      ...
      {paidAmount > targetAmount && <p className="mt-2 text-xs text-slate-500">Payment exceeds the current statement amount.</p>}
    </div>
  );
}
