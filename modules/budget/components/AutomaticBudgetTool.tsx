    ...
    <section className="rounded-[20px] border border-blue-400/15 bg-[#0D1722] p-5 md:p-6">
      ...
      {!mode ? (
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          <button type="button" onClick={() => setMode("OVERALL")} className="rounded-2xl border border-blue-400/15 bg-[#111C28] p-5 text-left transition hover:border-blue-400/30 hover:bg-[#132131]">
            <p className="text-sm font-semibold text-white">Overall budget</p>
            <p className="mt-1 text-xs text-slate-500">Calculate one monthly limit for all expense categories combined.</p>
          </button>
          <button type="button" onClick={() => setMode("CATEGORY")} className="rounded-2xl border border-blue-400/15 bg-[#111C28] p-5 text-left transition hover:border-blue-400/30 hover:bg-[#132131]">
            <p className="text-sm font-semibold text-white">Per category</p>
            <p className="mt-1 text-xs text-slate-500">Calculate separate recommendations for each expense category.</p>
          </button>
        </div>
      ) : (
        <>
          ...
          {!suggestion ? (
            <div className="mt-4 rounded-2xl border border-white/10 bg-[#111C28] p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-white">{mode === "OVERALL" ? "Overall budget recommendation" : "Category budget recommendations"}</p>
                  <p className="mt-1 text-xs text-slate-500">Review the calculation before applying it to this month.</p>
                </div>
                <button type="button" onClick={calculate} disabled={busy} className="rounded-xl bg-blue-500 px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-50">{busy ? "Calculating…" : mode === "CATEGORY" ? "Calculate categories" : "Calculate overall"}</button>
              </div>
            </div>
          ) : (
            <div className="mt-4 rounded-2xl border border-white/10 bg-[#111C28] p-4">
