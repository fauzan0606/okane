...
export default function ReconciliationClient({ wallets, session }: Props) {
  ...
  function complete() {
    if (!session) return;
    const form = new FormData(); form.set("sessionId", session.id);
    startTransition(async () => {
  ...
  return <div className="space-y-5">
    <form onSubmit={upload} className="rounded-[22px] border border-[#30465D] bg-[#172A3D] p-5 shadow-[0_14px_36px_rgba(0,0,0,0.18)]">
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-[10px] text-slate-600">PDF statements are read as documents so native text, tables, and scanned page visuals can be interpreted together.</p><button disabled={busy || !file || !walletId} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-xs font-bold text-[#07110b] disabled:cursor-not-allowed disabled:opacity-40">{busy ? <><LoaderCircle size={14} className="animate-spin" />Analyzing statement…</> : <><Upload size={14} />Analyze & compare</>}</button></div>
    ...
    {session && <section className="space-y-4">
      <div className="rounded-[22px] border border-[#30465D] bg-[#172A3D] p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] uppercase tracking-[0.14em] text-slate-500">2. Reconciliation result</p><h2 className="mt-1 text-lg font-semibold text-white">{session.fileName}</h2><p className="mt-1 text-xs text-slate-500">{session.wallet.name} · {session.sourceType === "CREDIT_CARD_STATEMENT" ? "Credit card statement" : "Bank statement"}</p></div>{session.status !== "COMPLETED" && <button onClick={complete} disabled={isPending || summary.pending > 0} className="rounded-xl bg-emerald-500 px-4 py-2.5 text-xs font-bold text-[#07110b] disabled:opacity-40">{isPending ? "Saving…" : summary.pending ? `Resolve ${summary.pending} rows first` : "Complete reconciliation"}</button>}</div>
