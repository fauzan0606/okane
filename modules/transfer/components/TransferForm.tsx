...
export default function TransferForm({ wallets }: { wallets: WalletOption[] }) {
  ...
  if (!open) return <button type="button" onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-xs font-bold text-[#06110b] transition hover:bg-emerald-400"><ArrowLeftRight size={15} /> + Transfer</button>;

  return <section className="rounded-[22px] border border-white/10 bg-[#0d151e] p-5 shadow-[0_18px_42px_rgba(0,0,0,0.16)] md:p-6">
    ...
    {wallets.length < 2 ? <div className="mt-5 rounded-xl border border-dashed border-amber-400/20 bg-amber-400/[0.04] p-4 text-sm text-amber-200">You need at least two active wallets before creating a transfer.</div> : <form action={async (formData) => { await createTransferAction(formData); setOpen(false); }} className="mt-5 grid gap-4 md:grid-cols-2">
      ...
      <div className="flex items-end gap-2"><button type="button" onClick={() => setOpen(false)} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 px-5 py-2.5 text-sm font-semibold text-slate-300 hover:bg-white/5"><X size={15} />Cancel</button><button type="submit" className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-500 px-5 py-2.5 text-sm font-bold text-[#06110b] hover:bg-emerald-400"><ArrowRight size={16} />Transfer</button></div>
    </form>}
  </section>;
}
