...
function SummaryCard({ icon, tone, title, value, detail }: { icon: React.ReactNode; tone: "green" | "orange" | "blue" | "purple"; title: string; value: string; detail: string }) { const toneClasses = { green: "bg-emerald-500/15 text-emerald-400", orange: "bg-orange-500/15 text-orange-400", blue: "bg-cyan-500/15 text-cyan-400", purple: "bg-violet-500/15 text-violet-400" }; return <div className="rounded-[18px] border border-white/10 bg-[#0d141e] px-4 py-4 shadow-[0_8px_30px_rgba(0,0,0,0.14)] transition hover:-translate-y-0.5 hover:border-white/15"><div className="flex items-start gap-3"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${toneClasses[tone]}`}>{icon}</div><div className="min-w-0"><p className="text-[11px] font-medium text-slate-400">{title}</p><p className="mt-1 truncate text-[17px] font-semibold tracking-tight text-white">{value}</p><p className="mt-1 text-[10px] text-slate-500">{detail}</p></div></div></div>; }
function DarkPanel({ children }: { children: React.ReactNode }) { return <div className="rounded-[20px] border border-white/10 bg-[#0d141e] p-5 shadow-[0_10px_35px_rgba(0,0,0,0.16)] md:p-6">{children}</div>; }
function EmptyState({ message }: { message: string }) { return <p className="py-7 text-sm text-slate-500">{message}</p>; }
