...
export default async function CreditCardSummary() {
  ...
  return <section className="mb-5">
    ...
    <div className="grid gap-3.5 md:grid-cols-2 xl:grid-cols-3">{summaries.map((card) => { const meta = statusMeta(card.status); const alert = card.status === "OVERDUE" || card.status === "UNPAID"; return <div key={card.id} className="rounded-[20px] border border-white/10 bg-[#121B26] p-5 shadow-[0_12px_35px_rgba(0,0,0,0.16)] transition hover:border-white/15 hover:bg-[#172231]">
