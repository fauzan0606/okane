...
function validateInput(input: SplitBillInput) {
  ...
  validateCharge(input.tax, "Tax"); validateCharge(input.serviceFee, "Service fee");
...
export async function finalizeSplitBill(splitBillId: string, input: { transactionDate: Date; walletId: string }) {
  const merchant = await prisma.splitBill.findUnique({ where: { id: splitBillId }, select: { merchantName: true } }); if (!merchant) throw new Error("Split Bill not found.");
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!splitBill) throw new Error("Split Bill not found."); if (splitBill.status !== SplitBillStatus.DRAFT && splitBill.status !== SplitBillStatus.OPEN) throw new Error("This Split Bill has already been finalized or cancelled."); if (splitBill.transactionId) throw new Error("This Split Bill is already linked to a transaction.");
    const wallet = await tx.wallet.findUnique({ where: { id: input.walletId }, select: { id: true, currencyId: true, balanceAsOf: true } }); if (!wallet) throw new Error("Wallet not found.");
    ...
    for (const participant of splitBill.participants) { if (participant.isMe || participant.shareAmount.lte(0) || participant.receivable) continue; await tx.receivable.create({ data: { personName: participant.name, description: `Split Bill: ${splitBill.merchantName}`, amount: participant.shareAmount, currencyId: wallet.currencyId, sourceWalletId: wallet.id, loanDate: input.transactionDate, sourceTransactionId: transaction.id, splitBillParticipantId: participant.id } }); }
    await tx.splitBill.update({ where: { id: splitBill.id }, data: { transactionId: transaction.id, status: SplitBillStatus.OPEN } }); return transaction;
  });
}
...
export async function deleteSplitBill(splitBillId: string) {
  return prisma.$transaction(async (tx) => {
    const splitBill = await tx.splitBill.findUnique({ where: { id: splitBillId }, include: { participants: { include: { receivable: { include: { payments: true } } } } } }); if (!splitBill) throw new Error("Split Bill not found.");
    if (splitBill.transactionId) throw new Error("This Split Bill is already in your financial records. Delete or correct the linked transaction and receivables from their respective modules instead.");
    const hasPayments = splitBill.participants.some((participant) => (participant.receivable?.payments.length ?? 0) > 0); if (hasPayments) throw new Error("Remove the receivable payment history before deleting this Split Bill.");
    const receivableIds = splitBill.participants.map((participant) => participant.receivable?.id).filter((id): id is string => Boolean(id)); if (receivableIds.length) await tx.receivable.deleteMany({ where: { id: { in: receivableIds } } }); await tx.splitBill.delete({ where: { id: splitBill.id } });
  });
}
