...
export default function SplitBillOcr({ onUseResult }: Props) {
  ...
  return <section className="rounded-[22px] border border-[#30465D] bg-[#172A3D] p-5 shadow-[0_14px_36px_rgba(0,0,0,0.22)]">
    ...
    {result && <div className="mt-4 rounded-2xl border border-emerald-400/10 bg-[#0B141F] p-4">
      <div><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-emerald-400">Gemini OCR Preview</p><p className="mt-1 text-xs text-slate-400">Receipt values are transcribed as printed. Line-item prices are not recalculated; discounts, tax treatment, and delivery adjustments stay separate.</p></div>
      <div className="mt-4 grid gap-3 md:grid-cols-4"><div><p className="text-[9px] uppercase tracking-[0.1em] text-slate-600">Merchant</p><p className="mt-1 truncate text-xs font-semibold text-white">{result.merchantName || "Not detected"}</p></div><div><p className="text-[9px] uppercase tracking-[0.1em] text-slate-600">Item Subtotal</p><p className="mt-1 text-xs text-slate-300">{money(result.subtotal)}</p></div><div><p className="text-[9px] uppercase tracking-[0.1em] text-slate-600">Tax / Service</p><p className="mt-1 text-xs text-slate-300">{money(result.taxAmount)} / {money(result.serviceAmount)}</p><p className="mt-1 text-[9px] text-slate-500">Tax {treatmentLabel(result.taxMode)} · Service {treatmentLabel(result.serviceMode)}</p></div><div><p className="text-[9px] uppercase tracking-[0.1em] text-slate-600">Grand Total</p><p className="mt-1 text-xs font-semibold text-white">{money(result.grandTotal)}</p></div></div>
      ...
      <div className="mt-3 rounded-lg border border-amber-400/10 bg-amber-400/[0.03] px-3 py-2 text-[10px] text-amber-200">{result.taxMode === "INCLUDED" || result.serviceMode === "INCLUDED" ? "Included charges will not be added again to the Split Bill total." : result.taxMode === "UNKNOWN" || result.serviceMode === "UNKNOWN" ? "Tax/service treatment could not be established from the receipt. Those charges will not be added automatically; review the invoice before using the result." : "OCR does not change invoice line-item prices to force the total to balance."}</div>
      ...
      <div className="mt-4 flex justify-end border-t border-white/5 pt-4"><button type="button" onClick={useResult} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-500 px-4 py-2 text-[10px] font-bold text-[#07110b]"><Check size={13} /> Use OCR result</button></div>
    </div>}
  </section>;
}
