...
export default function SplitBillForm({ currencySymbol = "Rp" }: Props) {
  ...
  const valid = merchantName.trim().length > 0 && participants.filter((participant) => !participant.isMe && participant.name.trim()).length >= 1 && items.length > 0 && validCharge(tax) && validCharge(serviceFee) && items.every((item) => item.name.trim() && Number(item.quantity) > 0 && Number(item.unitPrice) >= 0 && (item.splitMethod === "EQUAL" || item.units.some((unit) => Number(unit) > 0)));

  function addParticipant() { setParticipants((current) => [...current, { name: "", isMe: false }]); setItems((current) => current.map((item) => ({ ...item, units: [...item.units, "1"] }))); }
  ...
  function useOcrResult(result: OcrResult) {
    ...
    if (result.items.length > 0) {
      setItems(result.items.map((item) => ({ name: item.name, quantity: String(item.quantity), unitPrice: String(item.unitPrice), splitMethod: "EQUAL", units: participants.map(() => "1") })));
    }
    if (result.taxPercent !== undefined) setTax({ mode: "PERCENT", value: String(result.taxPercent) });
    if (result.servicePercent !== undefined) setServiceFee({ mode: "PERCENT", value: String(result.servicePercent) });
  }

  const payload = JSON.stringify({
    merchantName,
    participants: participants.map((participant) => ({ name: participant.name, isMe: participant.isMe })),
    items: items.map((item) => ({ name: item.name, quantity: Number(item.quantity), unitPrice: Number(item.unitPrice), splitMethod: item.splitMethod, units: item.units.map(Number) })),
