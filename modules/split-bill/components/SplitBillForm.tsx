...
export default function SplitBillForm({ currencySymbol = "Rp", onSaved }: Props) {
  ...
  return <form onSubmit={handleSubmit} className="space-y-5">
    ...
    </div>; })}</div>      <div className="mt-4 rounded-xl border border-emerald-400/10 bg-emerald-400/[0.03] p-3"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-emerald-400">Who paid?</p><div className="mt-2 grid gap-2 sm:grid-cols-[1fr_1fr] sm:items-center"><select value={payerIndex} onChange={(event) => setPayerIndex(Number(event.target.value))} className={inputClass}>{participants.map((participant, index) => <option key={index} value={index}>{participant.name || (participant.isMe ? "You" : `Person ${index + 1}`)}{participant.isMe ? " (You)" : ""}</option>)}</select><p className="text-[10px] leading-4 text-slate-500">If someone else paid, OKANE will not create an expense from your wallet when you finalize the Split Bill.</p></div></div>
</section>
