...
import { setFinalizedTransactionCategory } from "./finalization-category-service";
...
export async function finalizeSplitBillAction(formData: FormData) {
  ...
  if (typeof categoryId !== "string" || !categoryId) throw new Error("Category is required.");
  if (subcategoryId !== null && typeof subcategoryId !== "string") throw new Error("Invalid subcategory.");

  const transaction = await finalizeSplitBill(splitBillId, { transactionDate: new Date(transactionDate), walletId });
  await setFinalizedTransactionCategory(transaction.id, categoryId, subcategoryId || undefined);
  refreshAll();
}
