...
export default function InvestmentCashWithdrawPortal() {
  ...
  useEffect(() => {
    fetch("/api/investments/cash-transfer", { cache: "no-store" })
      ...
      .then((j) => {
        const nextCash = (j.cashAccounts ?? []) as CashAccount[];
        setWallets(j.wallets ?? []);
        setCashAccounts(nextCash);
        if (nextCash[0]) {
          setCashAccountId((current) => current || nextCash[0].id);
          setHistoryAccountId((current) => current || nextCash[0].id);
        }
      })
  ...
  async function submit() {
    ...
    try {
      ...
      setAmount(""); setHistoryAccountId(source.id); setHistoryOpen(true);
