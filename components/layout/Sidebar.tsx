...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden h-[100dvh] w-[248px] shrink-0 flex-col border-r border-white/10 bg-[#070c12] text-slate-100 md:flex">
      ...
      <nav className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-3 [scrollbar-width:thin]">
        {menus.map(group => (
          <div key={group.title} className="mb-6">
            ...
            <div className="space-y-1">
              {group.items.map(item => {
                ...
                return (
                  <Link key={item.label} href={item.href} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? "bg-emerald-500/20 text-white shadow-[inset_0_0_0_1px_rgba(52,211,153,0.08)]" : "text-slate-400 hover:bg-white/5 hover:text-slate-100"}`}>
                    <Icon size={18} strokeWidth={active ? 2.2 : 1.8} className={active ? "text-emerald-400" : "text-slate-500"} />
      ...
      <div className="shrink-0 border-t border-white/5 bg-[#070c12] p-4">
        <div className="mb-3 flex items-center gap-3 rounded-xl border border-white/10 bg-[#0d141e] px-3 py-3">
          ...
          <div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-slate-200">Fauzan</p><p className="text-[10px] font-medium text-amber-400">Premium Plan</p></div>
          <span className="text-slate-600">›</span>
        </div>
        <Link href="/settings" className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${pathname === "/settings" ? "bg-white/10 text-white" : "text-slate-500 hover:bg-white/5 hover:text-slate-200"}`}>
