    ...
    discounts: {
      ...
      items: {
        ...
        properties: {
          ...
          amount: { type: "number" },
          percent: { type: "number" },
          scope: { type: "string", enum: ["ORDER", "DELIVERY", "ITEM"] },
        },
        required: ["name", "amount", "scope"],
      },
    },
    subtotal: { type: "number" },
    taxPercent: { type: "number" },
    taxAmount: { type: "number" },
    taxMode: { type: "string", enum: ["INCLUDED", "EXCLUDED", "UNKNOWN"] },
    servicePercent: { type: "number" },
    serviceAmount: { type: "number" },
    serviceMode: { type: "string", enum: ["INCLUDED", "EXCLUDED", "UNKNOWN"] },
    deliveryFeeAmount: { type: "number" },
    deliveryDiscountAmount: { type: "number" },
...
function treatment(value: unknown): ChargeTreatment {
  return value === "INCLUDED" || value === "EXCLUDED" ? value : "UNKNOWN";
}

function discountScope(value: unknown): DiscountScope {
  return value === "DELIVERY" || value === "ITEM" ? value : "ORDER";
}

function closeEnough(a: number, b: number, tolerance = 1) {
  return Number.isFinite(a) && Number.isFinite(b) && Math.abs(a - b) <= tolerance;
}

function normalizeResult(raw: ReceiptResult): ReceiptResult {
  const items: ReceiptItem[] = Array.isArray(raw.items)
    ? raw.items.map((item: ReceiptItem) => {
  ...
  let discounts: ReceiptDiscount[] = Array.isArray(raw.discounts)
    ? raw.discounts.map((discount: ReceiptDiscount) => ({
        ...discount,
  ...
  if (deliveryFee > 0) {
    discounts = discounts.map((discount: ReceiptDiscount) => {
  ...
  const detectedDeliveryDiscount = discounts
    .filter((discount: ReceiptDiscount) => discount.scope === "DELIVERY")
    .reduce((sum: number, discount: ReceiptDiscount) => sum + (Number(discount.amount) || 0), 0);
  ...
  const itemSubtotal = round(items.reduce((sum: number, item: ReceiptItem) => sum + (Number(item.amount) || 0), 0));
  const orderDiscount = round(discounts.filter((discount: ReceiptDiscount) => discount.scope === "ORDER").reduce((sum: number, discount: ReceiptDiscount) => sum + (Number(discount.amount) || 0), 0));
  ...
  return {
    ...
    servicePercent: Number.isFinite(serviceAmount) && serviceAmount > 0 ? Number(raw.servicePercent) : undefined,
...
- The grand total is informational source-of-truth and must never be used to alter line-item prices.`;
  ...
  if (!response.ok) {
    ...
    try {
      ...
      // Keep the user-facing message generic when the provider does not return JSON.
    }
    ...
    return jsonError(`Gemini could not process this receipt.${providerMessage}`, 502);
  }
  ...
  if (!text) {
    const reason = candidate?.finishReason && candidate.finishReason !== "STOP" ? ` (${candidate.finishReason})` : "";
    return jsonError(`Gemini returned an empty OCR result${reason}.`, 502);
  }

  try {
    return NextResponse.json(normalizeResult(JSON.parse(text) as ReceiptResult));
  } catch {
    console.error("Gemini receipt OCR returned invalid JSON", text);
    return jsonError("Gemini returned an invalid OCR result.", 502);
  }
}
