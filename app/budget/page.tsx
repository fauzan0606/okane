...
import BudgetCategoryActions from "@/modules/budget/components/BudgetCategoryActions";
...
export default async function BudgetPage({ searchParams }: { searchParams: Promise<{ month?: string; currency?: string; edit?: string }> }) {
  ...
  const hasOverall = overview.mode === "OVERALL";
  ...
  return (
    <AppShell sidebar={<Sidebar />} header={<Header />}>
      <div className="space-y-6">
        ...
        <section className={`rounded-[20px] border p-4 md:p-5 ${overview.mode === "NONE" ? "border-amber-400/20 bg-[#171B1C]" : overview.mode === "OVERALL" ? "border-blue-400/15 bg-[#0E1722]" : "border-emerald-400/15 bg-[#0E1722]"}`}>
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">Budget status</p>
          <p className="mt-1 text-sm font-semibold text-white">{overview.mode === "NONE" ? "Anda belum membuat budget untuk bulan ini." : overview.mode === "OVERALL" ? "Anda sudah membuat overall budget untuk bulan ini." : `Anda sudah membuat budget per category untuk bulan ini (${overview.items.length} item).`}</p>
        </section>
        ...
        <section className="space-y-3">
          {overview.mode === "CATEGORY" && <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-[#0d141e] px-4 py-3"><div><p className="text-sm font-semibold text-white">Category budgets</p><p className="text-[10px] text-slate-500">Delete the complete category budget plan for this month.</p></div><BudgetCategoryActions month={month} currencyCode={overview.currency.code} /></div>}
          {overview.items.length === 0 ? <div className="rounded-[20px] border border-dashed border-white/10 bg-[#0d141e] p-10 text-center"><p className="text-sm font-semibold text-white">{overview.mode === "OVERALL" ? "Overall budget is active" : "No category budgets for this month yet"}</p><p className="mt-1 text-xs text-slate-500">{overview.mode === "OVERALL" ? "All expense categories are tracked against the single overall limit above." : "Choose the automatic planner or add a category limit manually."}</p></div> : overview.items.map((item) => <BudgetItemRow key={item.id} item={item} currencySymbol={overview.currency.symbol} editHref={`/budget?month=${month}&currency=${overview.currency.code}&edit=${item.id}`} />)}
        </section>
      </div>
    </AppShell>
  );
